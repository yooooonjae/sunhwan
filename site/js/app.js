...
(function () {
  ...
  function renderBunyang() {
    ...
    $("#b-ladder-cap").innerHTML = D.ladder.map(x => `${x.name} n=${x.n}`).join(" · ") +
      " — 금 = 초기분양률 중위 95% 이상. 시도·분기 평균의 관측된 상관관계이며 개별 단지 예측이 아니다.";
  ...
  function renderLinkage() {
    const L = B.linkage;
    if (!L || !$("#l-spread")) return;   // #l-kpis 제거 — 3개 연결 카드가 정적 마크업으로 대체(수치 직접)
    ...
    $("#l-notes").innerHTML =
      "· 연결 ①의 파이프라인은 <b>청약홈 공고 기반(2020-02~)</b>만 집계 — 공고 없이 입주하는 물량은 빠져 있어 절대량이 아니라 방향으로 읽는다.<br>" +
      "· 연결 ③은 분기 " + L.cap_link.length + "개·국내 오피스 리츠 " + L.office_n + "종의 짧은 표본 — 강한 상관(r=" + L.r_cap + ")이지만 두 변수 모두 금리라는 공통 원인의 영향을 받는다.<br>" +
      "· 강건성 검증 결과 — 연결 ①: 분기 차분 r = " + L.robust.supply_d1 + "(소멸), 연간 변화 r = " + L.robust.supply_yoy + "(유지) → 단기 잡음이 아닌 중기 수급 수준의 관계. " +
      "연결 ③: 금리 통제 부분상관 r = " + L.robust.cap_partial + "(유지), 분기 차분 r = " + L.robust.cap_d1 + "(소멸) → 금리만으로 설명되지 않는 수준 관계이나, 표본 13개로 단정할 수 없다.<br>" +
      "· 세 연결 전부 관측된 상관관계이며 인과관계를 뜻하지 않는다. 선행·후행(그레인저류) 검정과 시군구 단위 연결은 후속 고도화 대상이다.";
  }
