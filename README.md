...
> 시세·상업용 임대 지표는 자매 저장소 [수지](https://yoonjae.pages.dev)의 산출물을 참조한다. 기본 경로는 `~/개발`이며, 다른 위치에 두었다면 `SUJI_DIR` 환경변수로 지정한다 — 예: `SUJI_DIR=/path/to/수지 python3 src/build/site_data.py`.
