...
def common_base_quarter(hug, real_sidos=None):
    """지도 채색용 공통 기준분기 선택 (순수 함수 — 계약 테스트 대상).

    유효 지역(rate!=None) 커버리지가 80% 이상인 분기 중 가장 최신을 고른다.
    80%를 넘는 분기가 없으면 (최대 커버리지, 최신) 폴백.
    반환: {"q", "valid", "total", "coverage"} — 후보 없으면 q=None·coverage=0.
    """
    if real_sidos is None:
        real_sidos = [s for s in SIDO_ORDER if s != "전국"]
    ...
    return {"q": common_q, "valid": q_valid.get(common_q, 0), "total": total_sido,
...
def bunyang_block():
    ...
    common = common_base_quarter(hug, real_sidos)
    common_q = common["q"]
...
def pbv(mcap, equity):
    """주가순자산배율 P/BV = 시가총액 / 자기자본 (순수 함수 — 계약 테스트 대상)."""
    return round(mcap / equity, 2)


def div_yield(dps, close):
    """배당수익률(%) = 주당배당금 / 종가 × 100. 배당 없으면 None (순수 함수)."""
    return round(dps / close * 100, 1) if dps else None
...
def reits_block(as_of=None):
    ...
    for t, meta in R.items():
        ...
        items.append({
            ...
            "pb": pbv(last["mcap"], eq["equity"]),
            "dy": div_yield(dps, last["close"]),
            "ltv": debt_ratio(eq.get("liab"), eq.get("assets")),
